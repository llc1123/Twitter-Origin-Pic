...
function download() {
  ...
  if (site === 'twitter') {
    ...
    fn =
      localStorage['fn-twitter']
        .replaceAll('%picid%', content.picId)
...
function initialize() {
  if (localStorage.getItem('subfolder') === null)
    localStorage['subfolder'] = 'twitter-origin-pic'
  if (localStorage.getItem('fn-twitter') === null)
    localStorage['fn-twitter'] = '%filename%'
  if (localStorage.getItem('time-format') === null)
    localStorage['time-format'] = 'YYYY-MM-DD HH-mm-ss'
}
