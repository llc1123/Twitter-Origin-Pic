...
document.body.onmousedown = function (img){
	...
	if (img.target.parentNode.classList.contains("AdaptiveMedia-photoContainer")){
		...
		if (img.target.classList.contains("media-image")){
			...
			tweet.imgUrl = img.target.parentNode.querySelector(".media-image").src.replace(":large","");
			img.target.parentNode.querySelector(".GalleryNav--prev").onclick = function(a){
				if (a.target.classList.contains("enabled"))	picId -= 1;
			};
			img.target.parentNode.querySelector(".GalleryNav-handle--prev").onclick = function(a){
				if (a.target.classList.contains("enabled"))	picId -= 1;
			};
			img.target.parentNode.querySelector(".GalleryNav--next").onclick = function(a){
				if (a.target.classList.contains("enabled"))	picId += 1;
			};
			img.target.parentNode.querySelector(".GalleryNav-handle--next").onclick = function(a){
				if (a.target.classList.contains("enabled"))	picId += 1;
			};
		}
